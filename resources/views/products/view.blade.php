            ...
            <div class="row" data-toggle="lightbox-gallery">
                ...
                <div class="col-md-6 push-bit">
                    <div class="clearfix">
                        ...
                        <div class="price">
                            <span class="h4 compared-price"><strong><s>&#x20b1; <i>{{ empty($variants) ? $product->compared_at_price : '0.00'}}</i></s></strong></span>
                            <span class="h4 main-price"><strong>&#x20b1; <i>{{ empty($variants) ? $product->price : '0.00' }}</i> Sale</strong></span>
                        </div>
                        <div class="variant">
                            ...
                            <form action="{{ route('add_to_cart') }}" method="post" enctype="multipart/form-data" class="cart-form">
                                ...
                                <div class="row">
                                    @foreach($variants as $name => $values)
                                        <?php $select_name = 'variants_'.(string)$i; ?>
                                        <div class="col-md-6">
                                            <label for="{{ $name }}">{{ ucfirst($name) }}</label>
                                            <div class="form-group">
                                                <select name="{{ $select_name }}" id="{{ $select_name }}" class="form-control variant-select">
    ...
    <script type="text/javascript">
        ...
        @if (!empty($variants))
            function updatePrice() {
                <?php echo "let variants = ". json_encode($variants_list) . ";\n"; ?>
                console.log(variants);
                let total_variants = {{ $i }} - 1;
                let prices = 0;
                if (total_variants === 3) {
                    let var_1 = $('#variants_1').val();
                    let var_2 = $('#variants_2').val();
                    let var_3 = $('#variants_3').val();

                    prices = variants[var_1][var_2][var_3];
                } else if (total_variants === 2) {
                    let var_1 = $('#variants_1').val();
                    let var_2 = $('#variants_2').val();

                    prices = variants[var_1][var_2];
                } else {
                    let var_1 = $('#variants_1').val();

                    prices = variants[var_1];
                }

                var data = prices.split('-');
                $('.compared-price i').html(data[1]);
                $('.main-price i').html(data[0]);
            }
        @endif
